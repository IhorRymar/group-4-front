...
let data = date.sort((a, b) => b.date.split('.').reverse().join().localeCompare(a.date.split('.').reverse().join()));
  ...
  <>
            ...
            <tbody>
                ...
                data.map(({ date, transactionType, category, comment, amount, balance, _id }) => (
                      <BodyList key={_id}>
                        ...
                        <BodyItems>{transactionType === "income" ? "+": "-"}</BodyItems>
                        <BodyItems>{Categories(category)}</BodyItems>
                        <BodyItems>{comment || "---"}</BodyItems>
                        <BodyItems>{transactionType === "income" ? 
                            <SpanSum style={{color: `${ baseVars.colors.incomeText}`}}>{amount.toFixed(2)}</SpanSum> :
                            <SpanSum style={{color:`${ baseVars.colors.expensesText}`} }>{amount.toFixed(2)}</SpanSum>}</BodyItems>
    ...
    </GeneralContainer>  : <EmptyContainer/>}
